...
bool MyMIRDumper::doFinalization(Module &M) {
    outs() << "Starting output MIR Info... \n";
    // log name diff from IRlog
    std::fstream output("./MIRlog", std::ios::out | std::ios::trunc | std::ios::binary);
    ...
    output.close();
    outs() << "Finish of dumping MIR Info. \n";
    return false;
} 
